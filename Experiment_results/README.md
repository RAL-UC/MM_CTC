...
computed by the scripts in this directory. [file:41]
...
- Tables 2 and 3 (average tracking error and associated statistics). [file:41]
...
values reported in Tables 2 and 3. [file:41]
...
end-effector positions at the final sample &N&. [file:41]
...
\[
J_{\Omega} = \mathbb{E}\bigl[J_{\vec{x}}\bigr] \approx
\frac{1}{\Omega}\sum_{i=1}^{\Omega} J_{\vec{x}}^{(i)}
\]
with sample variance
\[
V\bigl[J_{\Omega}\bigr] \approx
\frac{1}{\Omega}\sum_{i=1}^{\Omega}
\bigl(J_{\vec{x}}^{(i)} - J_{\Omega}\bigr)^2
\]
and an approximate 95% confidence interval
\[
J_{\Omega} \pm
1.96\,\sqrt{ \frac{V\bigl[J_{\Omega}\bigr]}{\Omega} }.
\]
These quantities are reported in the “variance” and “confidence interval”
columns of the tables. [file:41]
...
\]
with sampling period &T_s& and time instants &t_k = k T_s&. [file:41]

The scripts in `Table 4` compute:
- ITAE for base translational position (m·s).
- ITAE for base rotation (rad·s).
- ITAE norm of the arm joint angles (rad·s). [file:41]
...
tests (Test 1 and Test 2), comparing the discrete CTC and PD controllers. [file:41]
...
- `Test 1/`
  - Data files:
    ...
    - `cpc_simlab_int.mat`, `pd_simlab_int.mat`  
      ...
      &e_{\text{rms}}& and closing error. [file:41]
  ...
  - Scripts:
    - `plot_xee_test1.m`  
      ...
      &e_{\text{rms}}(t)& for CTC and PD (Figure 12, upper part). [file:41]
    - `plot_Prms_test1.m`  
      Computes and plots the RMS power curves &P_{\text{rms}}(t)& for CTC and
      PD (Figure 12, lower part). [file:41]

- `Test 2/`
  - Scripts:
    - `plot_xee_test2.m`, `plot_Prms_test2.m`  
      Same purpose as for Test 1, but for the second laboratory scenario. [file:41]
